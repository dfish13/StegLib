...
void LoadFile(int& node_num, int& edge_num, int*& edges, int*& weights, char* filename)
{
    ...
    // Added by Duncan to allow graph to be passed in via stdin
    FILE *fp;
...
void PrintMatching(int node_num, GeomPerfectMatching* gpm)
{
    printf("Duncanwashere\n%d %d\n", node_num, node_num/2);
    int i, j;
    for (i=0; i<node_num; i++)
    {
        ...
        {
            GeomPerfectMatching::REAL len = gpm->Dist(i, j);
            if ( ((GeomPerfectMatching::REAL)1 / 2) == 0 ) printf("%d %d %d\n", i, j, len);
            else                                           printf("%d %d %f\n", i, j, (double)len);
        }
    }
}
...
int main(int argc, char* argv[])
{
	...
	for (i=1; i<argc; i++)
	{
		...
		switch (argv[i][1])
		{
			...
			case 'w':
				...
				break;
            // Added by Duncan to print result to stdout
            case 'P':
                if (argv[i][2]) ShowUsage();
                print_matching = true;
                break;
	...
	if (!geom_filename)
	{
        ...
        if (print_matching) PrintMatching(node_num, pm);
	...
	else
	{
        ...
        if (print_matching) PrintMatching(geom_node_num, gpm);
